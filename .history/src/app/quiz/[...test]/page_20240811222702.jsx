...
export default async function page({ params }) {
   ...
   console.log(data.mergedData[0].hour);
   console.log(data.mergedData.length);

   function hoursminsec(length) {
    var hourss = 0;
    if (length <= 50) {
        hourss = 1;
    } else if (length <= 100) {
    ...
    return { hour: hourss, mint: 0, sec: 0 };
}

    function calculateTimeDifference(actualDuration, length, usedTime) {
        
        var hms = hoursminsec(length);
        ...
        return { hour: hours,mint: minutes,sec: seconds, actualDuration  }
    }
    

    
    // Example usage
    const actualDuration = { hours: 3, minutes: 0, seconds: 0 };
    const usedTime = { hours: 0, minutes: 0, seconds: 55 };
    
    const result = calculateTimeDifference(actualDuration, usedTime);
    console.log(result); // Output: { hours: 2, minutes: 59, seconds: 5 }
            ...
            <div className="container">
                <div className="row">
                    ...
                    {data.type == "new" ? (
                        ...
                        <Saved testId={data.testId} param0={params.test[0]} param1={params.test[1]} param2={params.test[2]} param3={params.test[3]} data={data.mergedData} skiparr={data.skiparr} marks={data.marks} index={data.index} shuuflephase={data.shuuflephase}  durations={calculateTimeDifference({ hours: 0, minutes: 0, seconds: 0 }, data.mergedData.length, { hours: data.mergedData[0].hour, minutes: data.mergedData[0].mint, seconds: data.mergedData[0].sec})} hoursminsec={data.mergedData.length} ></Saved>
                    )}
